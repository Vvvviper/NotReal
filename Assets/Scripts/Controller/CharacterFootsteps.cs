...
namespace Controller {
	...
	public class CharacterFootsteps : MonoBehaviour {
		...
		[SerializeField] FirstPersonController characterController;
		...
		void Update() {
			...
			if (triggeredBy == TriggeredBy.TRAVELED_DISTANCE) {
				float speed = (characterController ? characterController.currentVelocity  : characterRigidbody.velocity).magnitude;
		...
		void OnDrawGizmos() {
			if(debugMode) {
				Gizmos.DrawWireSphere(currentGroundInfo.point, groundCheckRadius);
				Gizmos.color = Color.red;
				Gizmos.DrawRay(transform.position, Vector3.down * (groundCheckDistance + groundCheckRadius));
			}
		}
